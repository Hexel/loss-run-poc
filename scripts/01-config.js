        ...
        const DOCUMENT_PROCESSOR_APIKEY = 'YJ8o39NuCNyTQaNsOn2h03ma4ooNb9txGcrj4ZY9MfYh7y0CDlmvBOEuW1U0A48G';
        ...
        function getPolicyField(policy, fieldName) {
            ...
            return policy.querySelector(`[name="policies[${policyIndex}][${fieldName}]"]`);
        }

        function getCoverageInputs(policy, checkedOnly = false) {
            const policyIndex = getPolicyIndex(policy);
            const checkedSelector = checkedOnly ? ':checked' : '';
            return Array.from(policy.querySelectorAll(`[name="policies[${policyIndex}][linesOfCoverage][]"]${checkedSelector}`));
        }
        ...
        function getClaimField(policyIndex, claim, fieldName) {
            ...
            return claim.querySelector(`[name="policies[${policyIndex}][claims][${claimIndex}][${fieldName}]"]`);
        }
