...
function syncAddPolicyButtonLabel() {
  ...
  if (addPolicyButton) {
    addPolicyButton.innerHTML =
      '<i class="fa-solid fa-square-plus" aria-hidden="true"></i>' +
      (hasPolicies
        ? "Add another prior policy"
        : "Add prior policy information");
  }
}
...
function setAddClaimsButtonLabel(addClaimsButton, label) {
  ...
  addClaimsButton.innerHTML =
    '<i class="fa-solid fa-square-plus" aria-hidden="true"></i> ' + text;
}
...
function hasPolicyCollision(policy, insurer, effectiveDate) {
  return getPolicies().some((candidate) => {
    ...
    return (
      insurerMatches(insurerField ? insurerField.value : "", insurer) &&
      effectiveField &&
...
function syncPolicyActionButtons(policy) {
  ...
  const priorAllowed =
    ...
    !hasPolicyCollision(policy, insurer, priorCandidateDate);
  if (priorButton) {
  ...
  const renewalAllowed =
    ...
    !hasPolicyCollision(policy, insurer, expirationDate);
  if (renewalButton) {
    ...
    if (data.effectiveDate) {
      ...
      if (data.expirationDate) {
        const expirationYear =
          new Date(data.expirationDate + "T00:00:00").getFullYear() - 2000;
