        ...
        function createPolicyEntry(policyIndex, data = {}) {
            ...
            const coverageLines = Array.isArray(data.coverageLines) ? data.coverageLines : [];
                    ...
                    <div class="coverage-group form-field">
                        ...
                        <div class="multiselect">
                            ...
                            <div class="checkboxes">
                                ${APP_CONFIG.coverageLines.map(line => `
                                    <label>
        ...
        function addPriorPolicy(sourcePolicy) {
            ...
            const selectedLines = getCoverageInputs(sourcePolicy, true).map(item => item.value);
            ...
            if (!parsed.hasSuffix) {
                ...
                policies.forEach((entry) => {
                    ...
                    entryNumberField.value = parsedEntry.base + '-' + formatSuffix(parsedEntry.suffix + 1);
                });
                newPolicyNumber = base + '-00';
            } else {
        ...
        function syncCoverageSelect(policy) {
            ...
            const selectedLines = checkboxes.filter(item => item.checked).map(item => item.value);
        ...
        function getNextClaimIndex(policy) {
            const indexes = getClaims(policy).map(claim => Number(claim.dataset.claimIndex));
        ...
        function appendClaim(policy, claimData = {}) {
            ...
            claimList.appendChild(createClaimEntry(policyIndex, getNextClaimIndex(policy), claimData));
        ...
        function syncClaimsForm(policy) {
            ...
            setAddClaimsButtonLabel(addClaimsButton, hasClaims ? 'Add another claim' : 'Add claim');
        }
        ...
        const uploadFileInput = uploadForm && uploadForm.querySelector('input[type="file"]');
        ...
        function setUploadButtonLabel(fileName) {
            ...
            uploadFileNameSpan.textContent = fileName ? 'Upload file: ' + fileName : 'Upload loss run file';
        }

        function handleUploadFileChange() {
            const files = uploadFileInput.files ? Array.from(uploadFileInput.files) : [];
            setUploadButtonLabel(files.length === 1 ? files[0].name : files.length + ' files selected');
            if (files.length > 0) {
        ...
        function normalizeCoverageLines(lines) {
            ...
            return lines.map(line => mapping[line] || line).filter(line => line === 'Auto Liability' || line === 'Auto Physical Damage' || line === 'General Liability');
        }
        ...
        function getCarrierName(carrier) {
            ...
            if (Array.isArray(carrier)) return carrier.map(c => getCarrierName(c)).filter(Boolean).join(', ');
            return '';
        }
        ...
        function getValuationDate(output) {
            const value = output?.valuation_date
        ...
        function mergeRecord(existing, incoming, existingValuationDate, incomingValuationDate) {
        ...
        function mergeExtractionRecords() {
            ...
            uploadedFiles.forEach(fileRecord => {
                const policies = Array.isArray(fileRecord.output?.policies) ? fileRecord.output.policies : [];
                const claims = Array.isArray(fileRecord.output?.claims) ? fileRecord.output.claims : [];
                const valuationDate = fileRecord.valuationDate;

                policies.forEach((policy, index) => {
                    const key = getRecordKey(policy, 'policy_number', `file-${fileRecord.uploadOrder}-policy-${index}`);
                    const current = policyRecords.get(key);
                    policyRecords.set(key, current
                        ? { record: mergeRecord(current.record, policy, current.valuationTimestamp, fileRecord.valuationTimestamp), valuationTimestamp: Math.max(current.valuationTimestamp, fileRecord.valuationTimestamp) }
                        : { record: { ...policy }, valuationTimestamp: fileRecord.valuationTimestamp });
                });

                claims.forEach((claim, index) => {
                    const key = getRecordKey(claim, 'claim_number', `file-${fileRecord.uploadOrder}-claim-${index}`);
                    const current = claimRecords.get(key);
                    claimRecords.set(key, current
                        ? { record: mergeRecord(current.record, claim, current.valuationTimestamp, fileRecord.valuationTimestamp), valuationTimestamp: Math.max(current.valuationTimestamp, fileRecord.valuationTimestamp) }
                        : { record: { ...claim }, valuationTimestamp: fileRecord.valuationTimestamp });
                });
            });
        ...
        function createPoliciesFromLossRun(output = mergeExtractionRecords()) {
            ...
            policies.forEach((policy, index) => {
                ...
                const matchingClaims = claims.filter(claim => claim.policy_number === policy.policy_number);
        ...
        function renderUploadedFiles() {
            ...
            uploadedFilesBody.innerHTML = rows.map(fileRecord => `
        ...
        function sortUploadedFiles(key) {
            if (uploadedFilesSort.key === key) {
                uploadedFilesSort.direction = uploadedFilesSort.direction === 'asc' ? 'desc' : 'asc';
            } else {
        ...
        function previewUploadedFile(uploadOrder) {
            const fileRecord = uploadedFiles.find(item => item.uploadOrder === uploadOrder);
            if (fileRecord?.previewUrl) filePreviewIframe.src = fileRecord.previewUrl;
        }
        ...
        async function processUploadedFile(file, uploadOrder) {
            ...
            if (!completed) {
                ...
                const uploadResponse = await fetch(APP_CONFIG.documentApiUrl + '/document', {
                    method: 'POST',
                ...
                if (!uploadResponse.ok) {
                    ...
                    throw new Error(errorBody.message || 'The document processor rejected the upload.');
                }

                const accepted = await uploadResponse.json();
                if (!accepted.documentId) throw new Error('The upload response did not include a documentId.');
                completed = await pollDocument(accepted.documentId);
                if (!completed?.output) throw new Error('The loss run could not be extracted.');
                cacheExtraction(fileHash, { output: completed.output, documentId: accepted.documentId, status: completed.status });
            }
        ...
        async function uploadAndPopulateLossRun(event) {
            ...
            showLoading(files.length === 1 ? 'Uploading PDF...' : `Uploading ${files.length} PDFs...`);

            try {
                const newRecords = await Promise.all(files.map((file, index) => processUploadedFile(file, uploadedFiles.length + index)));
        ...
        async function pollDocument(documentId) {
            ...
            for (let attempt = 0; attempt < APP_CONFIG.pollAttempts; attempt += 1) {
                ...
                if (!response.ok) {
                    ...
                    throw new Error(errorBody.message || 'The document processor could not retrieve the extraction.');
                }
                ...
                if (document.status === 'FAILED') {
                    throw new Error(document.statusMessage || 'The document processor failed to extract the PDF.');
                }
                showLoading('Processing PDF... (' + String(attempt + 1) + ')');
                await sleep(APP_CONFIG.pollIntervalMs);
            }
            throw new Error('The document processor did not finish extracting the PDF in time.');
        }
