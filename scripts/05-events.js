        ...
        function handleContinue(event) {
            const shouldContinue = hasInsuranceHistoryInformation()
                || window.confirm('You have not entered any insurance history information. Are you sure you wish to continue?');
        ...
        function handlePolicyInput(event) {
            ...
            if (target.matches('input[name$="][details]"]') && claim) {
                ...
                const largeDetail = largeLossesList.querySelector('[data-policy-index="' + policyIndex + '"][data-claim-index="' + claimIndex + '"]');
        ...
        function handleLargeLossInput(event) {
            ...
            const policy = policyList.querySelector('[data-policy-index="' + policyIndex + '"]');
            if (!policy) return;

            const claim = getClaims(policy).find(item => Number(item.dataset.claimIndex) === claimIndex);
            const detailField = claim ? getClaimField(policyIndex, claim, 'details') : null;
        ...
        function closeCoverageMenus(event) {
            ...
            if (!clickedInside) {
                document.querySelectorAll('.checkboxes').forEach(item => item.classList.remove('open'));
            }
        }
        ...
        function initializeApp() {
            ...
            if (uploadForm) uploadForm.addEventListener('submit', uploadAndPopulateLossRun);
            if (uploadFileInput) uploadFileInput.addEventListener('change', handleUploadFileChange);
            if (uploadedFilesTable) uploadedFilesTable.addEventListener('click', handleUploadedFilesClick);
